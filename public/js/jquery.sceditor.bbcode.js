/**
 * Sceditor: v1.5.2
 * SEARCH FOR TRENT to see changes made to this file when replacing it
 */
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
		...
		var SCEditor = function (el, options) {
			...
			initEditor = function (startInSourceMode) {
				...
				$wysiwygEditor = $(
					'<iframe id="frame_'+options.id+'" frameborder="0" allowfullscreen="true"></iframe>' // TRENT (add: id="frame_'+options.id+'")
	...
	// This is the problem
			...
			base.expandToContent = function (ignoreMaxHeight) {
				...
				/*else if ((ignoreMaxHeight === false && height >= maxHeight) ) {
					//trent added this to expand sceditor to 'options.resizeMaxHeight' if content is greater then max editor height so users can see as much as possible
					//removed because chrome/safari sometimes would add height to sceditor text area
					base.height(maxHeight);
				}

				if (base.height() < 10){
					base.height(10);console.log(base.bbcode);
				}*/
			};
	...
	// (pre and post converstion?)
					...
					/******************* IMPORTANT ***************************  //Trent
					For ClassMarker to keep text consistent, we always remove formatting/text styles when pasting text
					Note: Also need to replace
					 *********************************************************************************************/


					var pasteddata = (pastearea.innerText||pastearea.textContent);


					/*
					if (pluginManager.hasHandler('toSource')) { //trent remove parsing to bbcode because we have no html
					...
					*/
					...
					/*
					if (pluginManager.hasHandler('toWysiwyg')) { //trent remove parsing to html
						pasteddata = pluginManager.callOnlyFirst(
					...
					*/
					...
					base.wysiwygEditorInsertText(pasteddata, null, true);//trent


				/* Original version FROM this file   Trent
				handlePasteData = function (elm, pastearea) {
					// fix any invalid nesting
					dom.fixNesting(pastearea);
					// TODO: Trigger custom paste event to allow filtering
				...
				/* Modified TRENT version - strip all formatting on paste into editor boxes */
				...
				//var pasteddata = pastearea.innerHTML;trent
				//var pasteddata = pastearea.innerText;//innertext does not work in firefox, use textContent instead
				var pasteddata = (pastearea.innerText||pastearea.textContent);

				//if(pluginManager.hasHandler('toSource'))trent
					//pasteddata = pluginManager.callOnlyFirst('toSource', pasteddata, $(pastearea));trent

				pastearea.parentNode.removeChild(pastearea);

				//if(pluginManager.hasHandler('toWysiwyg'))trent
					//pasteddata = pluginManager.callOnlyFirst('toWysiwyg', pasteddata, true);trent

				rangeHelper.restoreRange();
				//base.wysiwygEditorInsertHtml(pasteddata, null, true);trent
				base.wysiwygEditorInsertText(pasteddata, null, true);
				/*End trent version ********************/
			};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
		...
		var RangeHelper = function (w, d) {
			...
			/* TRENT START CHANGE : REMOVE ORIGINAL FUNCTION
			IE Throws error when using this and img/videos are in bbcode editor
			...
			/* TRENT CHANGE : REPLACE WITH THIS FUNCTION */
			base.selectedRange = function() {
				var	range, parent,
					sel = isW3C ? win.getSelection() : doc.selection;
			...
			/* TRENT END CHANGE : REPLACE WITH THIS FUNCTION */
...
/***/ function(module, exports, __webpack_require__) {
	...
	// TODO: move these styles into the CSS file
			...
			youtube:
				...
				'src="https://www.youtube-nocookie.com/embed/{id}?wmode=opaque" ' +
				'data-youtube-id="{id}" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>'
		};
...
/***/ function(module, exports, __webpack_require__) {
	...
	// Maybe just check the firstBlock contains both the start and end containers
			...
			code: {
				...
				tooltip: 'Highlight Code - Add code first, then highlight it and select this button'
			},
			// END_COMMAND
			...
			ltr: {
				...
				tooltip: 'Text Direction: Left-to-Right'  // TRENT CHANGE
			},
			// END_COMMAND
			...
			rtl: {
				...
				tooltip: 'Text Direction: Right-to-Left'  // TRENT CHANGE
			},
			// END_COMMAND
	...
	var bbcodes = {
		...
		youtube: {
			...
			html: '<iframe width="560" height="315" frameborder="0" ' +
				...
				'data-youtube-id="{0}" allow="autoplay; encrypted-media" allowfullscreen></iframe>'
		},
		// END_COMMAND
